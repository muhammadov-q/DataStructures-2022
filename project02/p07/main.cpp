...
{
    int mNumber;
    int mNumOfSolvedProblems;
    int mPenaltyTime;
    bool mHadAtLeastOneSubmission;
    vector<Problem> mProblems;

    Contestant(int number)
        : mNumber(number), mNumOfSolvedProblems(0), mPenaltyTime(0), mHadAtLeastOneSubmission(false), mProblems(9)
    {
    }
...
struct CmpContestants
{
    ...
    {
        if (c1.mNumOfSolvedProblems != c2.mNumOfSolvedProblems)
        {
            return c1.mNumOfSolvedProblems > c2.mNumOfSolvedProblems;
        }
...
int main()
{
    ...
    for (int test = 0; test < nTests; ++test)
    {
        ...
        for (const auto &c : contestants)
        {
            ...
            {
                cout << c.mNumber << " " << c.mNumOfSolvedProblems << " " << c.mPenaltyTime << "\n";
            }
        }
    }
}